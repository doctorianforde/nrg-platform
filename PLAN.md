...
| Answer-option cues in the AI bank | **300 fixed on staging** (Claude, 2026-09-24) — **you**: decide how to do the other 1,700 | The correct answer was the longest option in 83% of the 2,000, so picking the longest scored ~83% without reading the stem. 300 rewritten by hand (21%, 6% and 9% longest by batch). `scripts/fix-option-cues.ts` holds the validator and now the duplicate detector too. ~1,430 of the remaining 1,700 still carry the cue and realistically need an API key and a scripted pass — see `PROGRESS.md`. |
...
| Near-duplicate questions in the AI bank | **AI** (once someone says go) | Now measured: ~8% of viable candidates are near-duplicates (47 restatements of already-rewritten items, plus 82 duplicating each other, in one 1,556-item pool). `--fetch` skips them so batches do not repeat work, but no whole-bank dedup has run — the duplicates are still queued for Jade to review twice. |
