...
| Answer-option cues in the AI bank | **600 fixed on staging** (Claude, 2026-09-24) — **you**: decide how to do the other 1,400 | The correct answer was the longest option in 83% of the 2,000, so picking the longest scored ~83% without reading the stem. 600 rewritten by hand — 30% of the bank — at 21%, 6%, 9% and 17.7% longest by batch. `scripts/fix-option-cues.ts` holds the validator and the duplicate detector. ~1,100 of the remaining 1,400 still carry the cue and realistically need an API key and a scripted pass; 600 worked examples now exist to prompt it with. See `PROGRESS.md`. |
...
| Near-duplicate questions in the AI bank | **AI** (once someone says go) | ~8% of viable candidates, confirmed across two batches (132 skipped in the last one alone). `--fetch` skips them so batches do not repeat work, but the token-overlap detector only catches *reworded* duplicates — three pairs restating the same scenario in different words still got through into one batch. A whole-bank dedup needs semantic comparison, and the duplicates remain queued for Jade to review twice. |
