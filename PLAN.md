...
| Answer-option cues in the AI bank | **100 fixed on staging** (Claude, 2026-09-24) — **you**: decide how to do the other 1,900 | The correct answer was the longest option in 83% of the 2,000, so picking the longest scored ~83% without reading the stem. 100 rewritten by hand, now 21%. `scripts/fix-option-cues.ts` holds the validator that enforces it. The remaining 1,900 realistically need an API key and a scripted pass — see `PROGRESS.md`. |
...
| Near-duplicate questions in the AI bank | **AI** (once someone says go) | Several near-duplicates surfaced incidentally in a 100-item sample (two SVT items, two opioid overdoses, three DKA/potassium items). Mechanical to detect; no dedup pass has been run. |
