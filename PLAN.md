...
| Review the 2,000 AI-generated questions (currently `is_active=false`) | **You / Jade** (content judgment), **AI** (tooling) | AI generated them and can build the review UI/export, but a nursing-exam SME (you or Jade) has to actually judge clinical accuracy before anything goes live to students. This is the single biggest remaining content-risk item in Phase 1. Jade started on 2026-09-21 — 3 reviewed so far. |
| Answer-option cues in the AI bank | **Done on staging** (Claude, 2026-09-24) — **you**: decide when this goes to prod, and whether Jade re-reviews first | The correct answer was the longest option in **83%** of the 2,000, so picking the longest scored ~83% without reading the stem. All 1,545 items that carried the cue are rewritten — the whole bank now sits at **18.1% correct-is-longest against a 25% chance baseline**, with the length tell down from 43.1 to 2.3 chars. The 455 never touched never carried the cue (25.1%, exactly chance). `scripts/fix-option-cues.ts` holds the validator; backups in `scripts/data/`. Prod still has the original 2,000, inert and byte-identical. See `PROGRESS.md`. |
| Jade's two open review notes | **You / Jade** (decision) | "Regionally accepted practices rather than Trinidad and Tobago", and "questions should be in a taxonomy rather than a difficulty". Both change how questions are written or tagged, so they need deciding before the next rewrite pass. His third note — rationales must cover the wrong options — is done for the 100 and enforced by the validator. |
| Near-duplicate questions in the AI bank | **AI** (once someone says go) | ~10% of viable candidates and rising as the pool shrinks (274 skipped across the last two batches). **Now the largest quality problem in the AI bank, since the length cue is fixed.** `--fetch` skips them so batches do not repeat work, but the token-overlap detector only catches *reworded* duplicates — multiple pairs restate the same scenario in different words and get through every batch — two failure-to-thrive infants, three tuberculosis-isolation items, three panic-attack items and several advance-directive pairs among them. A whole-bank dedup needs semantic comparison, and the duplicates remain queued for Jade to review twice. |
