...
| Answer-option cues in the AI bank | **900 fixed on staging** (Claude, 2026-09-24) — **you**: decide how to do the other 1,100 | The correct answer was the longest option in 83% of the 2,000, so picking the longest scored ~83% without reading the stem. 900 rewritten by hand — 45% of the bank — at 21%, 6%, 9%, 17.7% and 21.7% longest by batch, against a 25% chance baseline. `scripts/fix-option-cues.ts` holds the validator and the duplicate detector. The remaining 1,100 still sit at 69% longest with a 35.6-char tell, and realistically need an API key and a scripted pass; 900 worked examples now exist to prompt it with. See `PROGRESS.md`. |
...
| Near-duplicate questions in the AI bank | **AI** (once someone says go) | ~8-10% of viable candidates, confirmed across three batches (140 skipped in the last one alone). `--fetch` skips them so batches do not repeat work, but the token-overlap detector only catches *reworded* duplicates — three pairs restating the same scenario in different words got through in each of the last two batches. A whole-bank dedup needs semantic comparison, and the duplicates remain queued for Jade to review twice. |
