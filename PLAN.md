...
| Answer-option cues in the AI bank | **200 fixed on staging** (Claude, 2026-09-24) — **you**: decide how to do the other 1,800 | The correct answer was the longest option in 83% of the 2,000, so picking the longest scored ~83% without reading the stem. 200 rewritten by hand (batch 1 now 21% longest, batch 2 6%). `scripts/fix-option-cues.ts` holds the validator that enforces it. 1,559 of the remaining 1,800 still carry the cue and realistically need an API key and a scripted pass — see `PROGRESS.md`. |
...
| Near-duplicate questions in the AI bank | **AI** (once someone says go) | Roughly 1 in 10 of the 200 items reviewed so far duplicates another question — three separate SVT items, three opioid overdoses, three DKA/potassium items, and pairs across both batches. Mechanical to detect. Worth running **before** paying to rewrite the remaining 1,800, or duplicates get rewritten twice. |
